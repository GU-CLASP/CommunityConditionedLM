...
messages are classified as English (94.6\% of the test set) and X of \num{510} communities %TODO
have more than half of their messages classified as English.\footnote{See \cref{sec:ethics} for futher
  discussion.}
...
We do release our trained community embeddings, but the dimensionality 
is low enough that we believe it would be impossible to reproduce 
personally identifiable information from them.

\paragraph{Language identification}
As discussed in section \cref{sec:data}, we decided not to filter our data for non-English 
comments.
...
In our case, filtering out messages classified as non-English would bias the 
dataset away from non-standard and code-switched language varieties,
which are of interest in the current work.

Nevertheless, the representations learned by our model are (necessarily)
relative to the set of communities we chose. 
Thus the learned representations for non-English communities tend to be more similar to each other 
than to other communities that use mostly English, even if their predominant languages are not the same.
