...
\title{Conditional Language Models for Community-Level Linguistic Variation}
...
\author{Bill Noble \and Jean-Philippe Bernardy\\
  Centre for Linguistic Theory and Studies in Probability (CLASP) \\
  Dept. of Philosophy, Linguistics and Theory of Science\\
  University of Gothenburg \\
\texttt{\{bill.noble@, jean.philippe.bernardy@\}.gu.se}}
...
We experimented with every possible value for $l_c$, the depth of the
community embedding, in a three-layer model ($l_c\in\{0,1,2,3\}$).
...
%performance than the best Transformer model. 
%This partly contradicts
%the fact that Transformer-based models are the ones used in
%state-of-the art models for mainstream tasks. 
%We propose that the
...
\newcommand{\modelboxplot}[4]{
  \addplot+[
    boxplot={draw position=#3, box extend=0.3}, 
    draw=#2, mark=*, mark options={fill=#2, scale=0.5}, solid, 
    pattern=#4,
    pattern color=#2,
    area legend] 
...
\begin{tikzpicture}
  \begin{axis}[name=ig,
    ...
    \foreach \lc in {0, ..., 3}{
      \modelboxplot{lstm-3-\lc-ig}{violet}{\lc+0.2}{crosshatch dots}
      \modelboxplot{transformer-3-\lc-ig}{teal}{\lc-0.2}{grid}
  ...
  \begin{axis}[name=ppl,
    ...
    \modelboxplot{lstm-3-ppl}{violet}{0.2}{crosshatch dots}
    \modelboxplot{transformer-3-ppl}{teal}{-.2}{grid}
  \end{axis}
\end{tikzpicture}
\caption{
...
\newcommand{\pairwisecossimbarplot}[1]{
    ...
    \addplot[draw=violet,pattern color=violet!30, pattern=crosshatch dots] table [y=lstm-sim] {floats/#1};
    \addplot[draw=teal,pattern color=teal!30, pattern=grid] table [y=transformer-sim] {floats/#1};
...
\begin{tikzpicture}
  \begin{groupplot}[group style={
    ...
    \addplot[draw=violet,pattern color=violet!30,pattern=crosshatch dots] table [y=lstm-sim] {floats/high_ling_high_soc.csv};
    \addplot[draw=teal,pattern color=teal!30,pattern=grid] table [y=transformer-sim] {floats/high_ling_high_soc.csv};
...
% general conclusion from this mode of analysis.
...
\emph{representation} of embeddings. That is, re-aligning the embeddings by applying a simple
rotation (orthogonal transformation) on either matrix widely changes
the \(||L-S||_F\) correlation metric.

% Another way to consider the same problem is that,
...
singular value \(\sigma_i\) (the elements of the diagonal matrix \(Σ\)), gives a measure of how much
...
value for random linguistic embeddings \(L'\). The 
distribution of \(d(L',S)\) for random embeddings is difficult to
compute analytically, but we can instead evaluate it using a Monte Carlo
method.
...
\citet{DelTredici2017}, use a modified
skip-gram model to community-level linguistic variation.
They show that lexical semantic variation occurs even across 
