...
which learn embedding of the communites they are conditioned on 
...
Our main result is that the the representation of communities built by
CCLM has a positive correlation with user co-occurrence patterns.
Even though kind of correlation is a core hypothesis of
socio-linguistics,\todo{what was this effect called?}  we believe that
this study is the first to test it on topic-organised
communities. Furthermore, it appears that our method (correlating
...
linguistic emdeddings \emph{for the same model}, we find no evidence
