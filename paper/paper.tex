...
recognize: the less linguistic diversity, \jp{easiest it is to model computationally ...}
% the more specific the language becomes and
...
trained for about half as long as the LSTM models (\cref{tab:model-results}, 
test epoch).
...
We also consider the information gain of each CCLM over its baseline
...
%}
\bn{I think this is stated correctly now (not the average in exp space)} 
...
\num{93.58} for the best LSTM model . %TODO make long table
...
\caption{
  Log-probability LMCC confusion matrix for the LSTM model with $l_c=1$. 
  Communities are sorted by indiscernability. (The first communities predominantly use a language other than English, and are thus very discernable.)}
\label{fig:confusion}
\end{figure}
...
\begin{figure*}
  ...
  \nextgroupplot[xlabel=$\IG_{M_j}$, ylabel=$\Ind_j$]
      \addplot+[%
        only marks,
        mark=*,
        mark options={scale=0.35, solid, color=violet!80, fill=violet},
...
Further, we can apply a permutation to \(U\), \(V\) and \(Σ\) such that \(σ_i < σ_j\) iff \(i < j\). Doing so, the largest singular value \(σ_0\) corresponds to the principal
...
a mean \(μ_d\)=TODO and a standard deviation \(σ_d\)=TODO.\jp{Unfortunate overloading}
