...
We experiment with two kinds of model architecture: LSTM \citep{Hochreiter1997}\jp{Is it really a simple or a bi-lstm? Even If it's simple it may be worth specifying due to the prevalence of bilstm everywhere.}
...
for some communities, perplexity of the conditioned models is a lot lower,\bn{is this ok? took ``test'' out but made clear in the caption that it's the test set}%\jp{and training?}
whereas for others it is the same or even slightly higher (\cref{tab:model-ppls}).\bn{talk about this in terms of mutual information instead?}
...
range of people. \bn{I really mean this as a quality of the topic, rather than the community itself, although the two are probably correlated}% \jp{Is there a way to quantify general interest topics? Community size in terms of number of users?}
...
On the other hand, when we consider the \emph{information gain}\bn{define earlier}
of the CCLM over the corresponding (same encoder architecture)
un-conditioned 
...
We find a weak correlation (TODO; r , p) between the perplexity of the
CCLM and the indiscernibility of the corresponding community
(\cref{hyp:rich-harder-to-identify}). Because we have relatively few,
...
all communities, varies between 78.64 and 80.70. It is steadily
increasing when $l_c$ decreases.  For transformer models, it varies
between 89.38 and 122.56. Here perplexity per-word is not a monotonous
...
The representation of communities built by CCLM is exhibiting a clear
correlation with user-coocurrence patterns
(\cref{hyp:extra-linguistic-correlation}, TODO; r , p). A natural
