...
whereas for others it is the same or even slightly higher (\cref{tab:model-ppls}).\bn{talk about this in terms of mutual information instead?}\jp{This should make no difference (but I'm not sure on how you are taking averages; see other comment.)}
...
\jp{What we had before: average entropy gain is $H(m)-H_0(m)$, and
  ...
  $avg(H(m)-H_0(m)) = avg(H(m))-avg(H_0(m))$. Taking the exponential
...
to \citet{Kumar2018} % JP: I actually cannot find in their paper what they do.
