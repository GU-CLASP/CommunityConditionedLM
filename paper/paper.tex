...
% However, we are not so much interested in the ability of the CCLM to precisely model the language used in a community.

We now turn our interest to the Bayesian classifier induced by a CCLM, which we call a language-model-based community classifier
(LMCC). Indeed, the purpose of an LMCC is to estimate the probability $P(m|c_i)$ that a message $m$
belong in a community $c_i$. Using the Bayes theorem, we can turn a
...
$P(m | c_i)$ is given by the CCLM. $P(m)$ is the absolute
