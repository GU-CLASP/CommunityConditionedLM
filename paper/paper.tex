...
% \citep{Hovy2015,Yang2017}. % Move this to related work ?
...
\num{93.58} for the best conditional LSTM model. %TODO make long table
...
\jp{Conclusion: perplexity (linguistic complexity) is not correlated
  ...
  \section{Social Analysis of CCLM community embeddings (TODO: better title)}\label{sec:embedding-analysis}
...
decomposition \(U^TΣV = LS^T\). This way, the vectors of \(U\) and \(V\) give
...
Additionally, assuming a test community \(c\) and a CCLM embedding $x_c$ for it, we can make a prediction
of its social embedding to be $\hat y_c = V^TUx_c$ and expect a cosine similarity with the actual social embedding $y_c$ to be on average
$1 - d(L,S) / n$. 
...
to perfect independence), where $n=510$ is the number of communities
considered.
%
For our various embiddings, we observe a distance \(d(L,S)\) between
232 and 354, which corresponds to an expected cosine similarity beween
0.5 and 0.54. This strongly suggests that much information is shared
between social-network embeddings and linguistic embeddings.

However, to get a more precise sense of whether this correlation is
statistically significant, we have experimentally computed \(d(L,S)\)
for 10 random ``embeddings'' with the same shape as the CCLM
embeddings.  We observed a mean of \(μ_d=431.39\) and a
(Bessel's-corrected) standard deviation \(s_d=2.90\) in their distance
from the social embedding,
\(S\). %sigma vs. s notation: it seems that sample stdev is often written with an $s$ to differentiate from population stdev

Because the value of \(d(L,S)\) that we get for actual embedding is
between 61 and 68 standard deviations
(\cref{tab:embedding-correlations}), we can safely conclude that there
is statistically significant correlation between \(L\) and \(S\).

% Further, it is clear that the CCLM embedding
...
% (Useful to know, but may lead the reviewers astray)
