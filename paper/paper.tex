...
standard deviation \(s_d=3.94\) in their distance from the social embedding, \(S\). %\jp{Unfortunate overloading} luckily it seems that sample stdev is often written with an $s$ to differentiate from population stdev


Thus if \(d(L,S)\) is below \(419.56\)
($\mu_d - 3s_d$), 
...
\toprule
                        &   & LSTM     & Transformer \\
\midrule
  \multirow{4}{*}{$l_c$}  & 0 &         254.06  (44.99) &         239.41  (49.09)  \\
                          & 1 &         245.14  (47.25) & \textbf{232.18} (50.38)   \\
                          & 2 &         249.17  (46.23) &         233.47  (50.21)  \\
                          & 3 & \textbf{241.13} (48.27) &         237.74   (49.13) \\
\bottomrule
\end{tabular}
...
from the mean distance of our random embedding sample.} \label{tab:embedding-correlations}
\end{table}
