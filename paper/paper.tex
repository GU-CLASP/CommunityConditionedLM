...
For $c=0$\jp{it's unfortunate that the letter $c$ is clashing. To Fix!}, the community embedding is concatenated with the token embedding.
...
for some communities, the test \jp{and training?}perplexity of the conditioned models is a lot lower (Table~\ref{tab:model-ppls},
...
would tend to have more specialized vocabulary and speech\jp{can you say speech for written language?} patterns.
...
who generated them from the comment-community co-occurence matrix,
...
something like an F-score, but we prefer to use the perplexity metric
(again), which is has stronger information-theoretic grounds.
However, this time, the perplexity corresponds to the difficulty of
identifying a community, not predicting a word.
Technically, we consider the average perplexity of the LMCC for a
...
We call \(\Ind_j\) the (linguistic) indiscernibility of the community
$c_j$, and it is a measure of how unsure a model is about which
community a message belongs to --- when the message in question is
actually coming from a community $c_j$.
...
\jp{We could also compute the perplexity for Cij = snap cosine similarity w/ softmax.}
