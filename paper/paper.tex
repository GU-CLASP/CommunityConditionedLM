...
Not only is linguistic variation an object of inquiry in its own right,
but also important to consider in the development of robust, equitable language technology.
Previous work has shown that by taking demographic information into account, 
models can achieve better performance on various standard NLP tasks \citep{Hovy2015,Yang2017},
but little is known about how variation at the level of speech community affects performance.

There is a fair amount of previous work that investigates linguistic variation using computational methods.
However, most of this work considers variation across macro-social categories,
such as gender \citep{Burger2011,Ciot2013,Bamman2014}, age \citep{Nguyen2013}, 
and geographic location \citep{Eisenstein2010,Bamman2014a}.
An exception to this is \citet{DelTredici2017},
who use the modified skip-gram model of \citet{Bamman2014a} to show that lexical semantic
variation occurs across different communities organized around the same topic.

In the present work, we continue this line of 
investigating linguistic variation across online communities.
In contrast to previous work, we investigate variation across speech communities
(rather than macro-social categories), and use context-sensitive sequence models
...
In general, transformer models trained for about half as long as the LSTM models (test epoch, \cref{tab:model-results}).
...
\[\Ppl_M = \avg_{m\in M}(-e^{H(m)})\] \bn{Following the logic from IG, should we change Ppl to the exponential of the average cross entropy, rather than using the average of the message-level perplexity?} 
As one would expect, 
...
This is similar to how our model improves experiences improved performance
...
Along these lines, \citep{OConnor2010} uses a Bayesian generative model to
infer communities from variation in text data.
In contrast to our work, this model treats words as
independent events, ignoring the structure (and variation) in the
construction of sequences.
It does further suggest, however, that community-level variation
could be modeled in an unsupervised way.
...
We cannot exclude a correlation (\cref{tab:ind-corrs}) between the perplexity of the
...
(out of a maximum of 46)\jp{make consistent with data once the plot is stablilized}, so they retain fairly specific patterns:
...
expect non-monotonicity, and leave studying this effect to future work.

The pattern of information gain by community is similar across architectures.
Communities that benefit most from the conditioned model behave that way for 
