...
Computing this distance is known as the Procrustes problem. Its
solution is

\[d(L,S) = \mathsf {Tr} Σ\], where the \(Σ\) is obtained by the singular value
decomposition \(U^TΣV = LS^T\).  The vectors of \(U\) and \(V\) give
...
direction of correlation (\(U_0\), \(V_0\)), \(σ_1\) to the second
principal direction, etc. 

The \(d(L,S)\) metric varies between \(0\) (perfect independence) and
\(n\) (perfect correlation, obtained for example if \(L=S\)).

To get a sense of what value of \(d(L,S)\) indicates significant correlation,
...
Thus \(d(L,S)\) above \(μ+3σ\) can be safely considered to indicate a
