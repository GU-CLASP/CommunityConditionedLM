...
While the performance ($\Ppl_{M_j}$) of the LSTM models is better,
...
\begin{tikzpicture}
  ...
  \end{axis}
  \begin{axis}[name=ppl,
    anchor=north west,at={($(ig.south west)-(0,1cm)$)},
...
for some communities, $\IG_{M_j}$ is around \num{1}, or even slightly lower,
meaning that the CCLM has no advantage over the unconditioned baseline.
For other communities it is much higher, meaning that the CCLM performs better 
...
%\begin{tikzpicture}
%\begin{axis}
%\commscatter[]{lstm-3-1_ppl}{lstm-3-1_ig}
%\end{axis}
%\end{tikzpicture}
%\caption{
...
%\begin{figure*}
  ...
  %]
    %\nextgroupplot
    %\commscatter[]{lstm-3-1_ppl}{lstm-3-1_indisc}
    %\nextgroupplot
    %\commscatter[]{lstm-3-1_ig}{lstm-3-1_indisc}
