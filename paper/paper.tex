...
phenomenon.  For this purpose, we define (\cref{sec:cclm}) various Community Conditioned Language Models 
...
conclution (\cref{sec:discussion-conclusion}):
\begin{hypotheses}
...
(\cref{fig:comm-info-gain})\jp{missing label}.\footnote{
...
\begin{axis}[%
  ...
  \addplot+[%
    ...
    font=\tiny,
  ] table [x={lstm-3-ppl}, y={lstm-3-3-ig}] {floats/comm.csv};
\end{axis}
\end{tikzpicture}
\caption{
...
\begin{figure*}
  ...
  \nextgroupplot[xlabel=$\Ppl_{M_j}$, ylabel=$\Ind_j$]
      \addplot+[%
        ...
        mark options={scale=0.35, solid, color=violet!80, fill=violet},
        font=\tiny,
      ] table [x={lstm-3-1-ppl}, y={lstm-3-1-indisc}] {floats/comm.csv};
  \nextgroupplot[xlabel=$\IG_{M_j}$, ylabel=$\Ind_j$, xmax=1.6]
      \addplot+[%
        ...
        mark options={scale=0.35, solid, color=violet!80, fill=violet},
        font=\tiny,
      ] table [x={lstm-3-1-ig}, y={lstm-3-1-indisc}] {floats/comm.csv};
  \end{groupplot}
  \end{tikzpicture}
  \caption{%
...
eliminate TODO (a percentatge?) of communities as possible origins \bn{43\%? JP: yes}
for it. Furthermore, the information gain provided by
community-conditioning is a good predictor of how specific the
community is.\jp{By empirical observation?}
...
standard deviation \(s_d=2.90\) in their distance from the social embedding, \(S\). % luckily it seems that sample stdev is often written with an $s$ to differentiate from population stdev
...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}
    ...
    \end{groupplot}
  \end{tikzpicture}
  \captionof{figure}{First two components of the aligned social (left) and linguistic (right) embeddings,
    ...
    % JP: commented out this sentence which does not seem to refer to something visible.
    % JP: If the legend shows in the supplementary material, then the whole figure should be repeated there.
