...
Our dataset includes \emph{comments}\jp{Comments/Post terminology; make sure its consistens} from \num{46} subreddits, 
...
We tokenized the comments using the default English model for the SpaCy tokenizer Version 2.2.3\jp{citation?}
and lower-cased all tokens.

\subsection{Models}

At the the core of the community-conditioned language model (CCLM) 
is a standard $n$\jp{with n=3}-layer neural sequence encoder
...
We used\jp{past tense is a bit awkward?} a vocabulary size of \num{40000} tokens, including a special out-of-vocabulary token.
...
We used a batch size of \num{256} and a maximum sequence length of \num{64} tokens.\jp{What happens with longer sequences? Are sequences split at sentence boundaries?}
...
We trained the models until the validation loss stopped decreasing for two epochs in a row.
\jp{How many epochs does that turn out to be? Is there a meaningful difference between lstm/}
