...
variation in the level of linguistic complexity\jp{maybe revise terminology? Perhaps define linguistic complexity as perplexity?} exhibited by different
communities.
%\jp{
...
community, and $m$ stands for a message. \jp{what does \(p\) stand for?}
...
Thus if \(d(L,S)\) is below \(μ_d-3σ_d\), we can safely assume that there % below, because we have a distance rather than correlation in this convention.
