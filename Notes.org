...
Experiments:
 ...
 - test if the model can predict which community a message is coming from
   - leading idea:
     ...
     - entropy of W(i) is low if the community i is very specific (so messages can be easily identified as comming from a specific community.).
...
- Correlation between embeddings of several models.
  - Basic idea: check the Pearson correlation beween <c^m_i, c^m_j> for every
